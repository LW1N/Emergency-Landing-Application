...
int main(int argc, char **argv)
{
  ...
  for ( unsigned int i = 0; i < facilities.size() && count < 10; i++ )
  {
    ...
    /*
    auto it = find_if(runways.begin(), runways.end(), SiteNumber(a->site_number(), min_runway_length));

    while(it != end(runways)){
	good_runways.push_back(*it);
	it++;
	it = find_if(it, runways.end(), SiteNumber(a->site_number(), min_runway_length));
    }
    */
